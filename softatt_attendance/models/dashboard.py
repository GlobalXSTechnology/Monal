...
class saAttendanceDashboard(models.Model):
    ...
    def _get_dashboard_summary(self, _time=None, location_ids=None, department_ids=None, shift_ids=None, employee_ids=None):
        ...
        att_records     = self.env['hr.attendance'].search(att_domain+[('employee_id.company_id','=',self.env.company.id)])
        total           = self.env['hr.employee'].search_count(emp_domain+[('company_id','=',self.env.company.id)])
    ...
    def get_present_employee(self, _time=None):
        ...
        if not user_tz:
            raise ValidationError("Please Set up Your Timezone")
        now             = fields.Datetime.now() if not _time else _time
        total           =  self.env['hr.employee'].search_count([('company_id','=',self.env.company.id)])
        employees       = set(self.env['hr.attendance'].search([('employee_id.company_id','=',self.env.company.id),
    ...
    def get_absent_employee(self, _time=None):
        ...
        total_employees         = self.env['hr.employee'].search_count([('company_id','=',self.env.company.id),('resource_calendar_id.id', 'in', shift_ids)])
        absent_emps             = self.env['hr.employee'].search([('company_id','=',self.env.company.id),('id','not in', tuple(employees)), ('resource_calendar_id.id', 'in', shift_ids)]).ids
    ...
    def absent_employee_per_location(self):
        ...
        employees       = set(self.env['hr.attendance'].search([('employee_id.company_id','=',self.env.company.id),
            ...
            ('check_in', '>=', datetime.combine(date_utils._softatt_localize(now, user_tz).date(), datetime.min.time())), 
            ('check_in', '<=',datetime.combine(date_utils._softatt_localize(now, user_tz).date(), datetime.max.time()))]).mapped('employee_id.id'))
        absent_emps             = self.env['hr.employee'].read_group(domain=[('company_id','=',self.env.company.id),('id','not in', tuple(employees)), ('resource_calendar_id.id', 'in', shift_ids)],
    ...
    def get_last_ten_logs(self):
        ...
        last_ten_logs       = self.env['sa.attendance.log'].search([
            ('employee_id', '!=',   False),('employee_id.company_id','=',self.env.company.id),
    ...
    def get_late_today(self):
        ...
        if not user_tz:
            raise ValidationError("Please Set up Your Timezone")
        late_today          = self.env['hr.attendance'].search([('employee_id.company_id','=',self.env.company.id),
