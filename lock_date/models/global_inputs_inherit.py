...
class HrPayslip(models.Model):
            ...
            for val in vals_list:
                ...
                if date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Inputs before {lock_date.lock_date} cannot be created.")
    ...
    #                 date_in = fields.Date.to_date(vals['date_from'])
    #                 if date_in < lock_date.lock_date:
    #                     raise UserError(
    #                         f"Inputs cannot be created before lock date {lock_date.lock_date}"
    #                     )
    #     return super().create(vals_list)
    ...
    def write(self, vals):
        ...
        if lock_date:
            for rec in self:
                if rec.date_from:
                    date_in = rec.date_from
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Inputs before {lock_date.lock_date} cannot be modified.")
        ...
        if lock_date:
            for rec in self:
                if rec.date_from:
                    date_in = rec.date_from
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Inputs before {lock_date.lock_date} cannot be deleted.")
        return super().unlink()