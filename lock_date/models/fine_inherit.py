            ...
            for val in vals_list:
                ...
                if date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Fine records before {lock_date.lock_date} cannot be created.")
        ...
        if lock_date:
            for rec in self:
                ...
                if new_date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Fine records before {lock_date.lock_date} cannot be modified.")
        ...
        if lock_date:
            for rec in self:
                if rec.date:
                    date_in = rec.date
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Fine records before {lock_date.lock_date} cannot be deleted.")
        return super().unlink()