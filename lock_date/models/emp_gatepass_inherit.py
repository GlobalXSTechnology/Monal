            ...
            for val in vals_list:
                ...
                if date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Gatepass records before {lock_date.lock_date} cannot be created.")
        ...
        if lock_date:
            for rec in self:
                if rec.date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Gatepass records before {lock_date.lock_date} cannot be modified.")
        ...
        if lock_date:
            for rec in self:
                if rec.date:
                    date_in = fields.Date.to_date(rec.date)
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Gatepass records before {lock_date.lock_date} cannot be deleted.")
        return super().unlink()