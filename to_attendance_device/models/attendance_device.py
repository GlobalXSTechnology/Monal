...
class AttendanceDevice(models.Model):
    ...
    recipient_ids = fields.Many2many('hr.employee', 'recipient_idssssssssss_rel',string="Responsible")
    ...
    platform = fields.Char(string='Platform', help="The Platform of the device which will be filled automatically when you hit the 'Get Device Info' button.")
    fingerprint_algorithm = fields.Char(string='Fingerprint Algorithm',
    ...
    user_attendance_ids = fields.One2many('user.attendance', 'device_id', string='Attendance Data')
    total_att_records = fields.Integer(string='Attendance Records', compute='_compute_total_attendance_records')
    finger_template_ids = fields.One2many('finger.template', 'device_id', string='Finger Template')
    ...
    def connect(self):
        ...
        try:
            ...
            _logger.info('fffffffffffffffffffffffffff')
            _logger.info('fffffffffffffffffffffffffff')
            _logger.info('fffffffffffffffffffffffffff')
            _logger.info('fffffffffffffffffffffffffff')
            _logger.info('fffffffffffffffffffffffffff')
        ...
        if error_msg:
            company_email = self.env['res.company'].search([('id', '=', self.env.company.id)]).email
            email_to = ''
            for email in self.recipient_ids.mapped('work_email'):
                email_to += email + ','
            data = {
                'email_from': company_email,
                'email_to': email_to,
                'subject': 'Attendance Device Connectivity',

                'body_html': f"""<div>
                                                <p>Dear Sir/Mam,</p>
                                                <p></p>
                                                <p>The attendance device <b>{self.name}-{self.location_id.name}</b> is not connected.</p>
                                                <p>You are requested to check the device and inform back to HR.</p>
                                                </div>
                                                        """
            }
                        ...
                        for user in users:
                            ...
                            if len(users) % 2 != 0:
                                if len(users) >= 2:
                                    ...
                                    timestamp = datetime.combine(check_in_xz.date(), time(4, 0))
                                    count_18 = timestamp.total_seconds() / 3600
                                    diff = relativedelta.relativedelta(str_utc_timestamp_add
                                                                       , users[0].timestamp)
                                    diff_in_months = diff.months + diff.years * 12
